...
let darkFog = new THREE.Fog(0x000000, 1, 100);
scene.fog = darkFog;
const controls = new OrbitControls(camera, renderer.domElement);
let vrController;
let vrController2;
let cylinderMaterial;
function createVRController(controllerId) {
    const cylinderGeometry = new THREE.CylinderGeometry(0.025, 0.025, 1, 8);
...
const planeGeo = new THREE.PlaneBufferGeometry(250, 250, 100, 100);
const material = new THREE.MeshLambertMaterial({ color: 0x320088, wireframe: false, });
const planeMaterial = new THREE.MeshLambertMaterial({ color: 0x320011 });
planeMaterial.wireframe = false;
const cube = new THREE.Mesh(geometry, material);
scene.add(cube);
cube.scale.set(2, 2, 2);
const terrain = new THREE.Mesh(planeGeo, planeMaterial);
...
const pointLight = new THREE.PointLight(0xffffff, 12, 32);
pointLight.position.y = 20;
scene.add(pointLight);
const sphereHelperSize = 1;
const pointLightHelper = new THREE.PointLightHelper(pointLight, sphereHelperSize);
